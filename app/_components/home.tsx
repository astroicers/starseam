...
function SectionHeading({ head, id }: { head: SectionHead; id: string }) {
  return (
    <>
      ...
      <h2 className="mt-3 text-[21px] tracking-[var(--ss-track-head)]">{head.title}</h2>
      {head.intro ? (
        <p className="mt-3 max-w-[620px] text-[13.5px] leading-[1.9] text-[var(--ss-text-2)]">
          {head.intro}
        </p>
      ) : null}
    </>
...
export function Home({ dict, locale }: { dict: Dictionary; locale: Locale }) {
  ...
  return (
    <main className="mx-auto max-w-[880px] px-6 py-10">
      ...
      {/* ---------------------------------------------------------------- */}
      <section id="readings" className="mt-16 scroll-mt-8">
      ...
      {/* ---------------------------------------------------------------- */}
      <section id="surfaces" className="mt-16 scroll-mt-8">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {(
              ...
              <Plate key={elevation} elevation={elevation}>
                ...
                <p className="mt-2 text-[12.5px] text-[var(--ss-text-2)]">{caption}</p>
              </Plate>
            ))}
          </div>
        </Specimen>
          ...
          <Lames>
            <Plate elevation="float">
              <div className="flex items-baseline justify-between gap-4">
                <h3 className="text-[15px] tracking-[0.08em]">{d.surfaces.lames.cyTitle}</h3>
            ...
            <Plate elevation="raised">
              <h3 className="text-[15px] tracking-[0.08em]">{d.surfaces.lames.merakTitle}</h3>
              <p className="mt-2 text-[12.5px] text-[var(--ss-text-2)]">
                {d.surfaces.lames.merakBody}
              </p>
            </Plate>
            <Plate elevation="flush">
              <h3 className="text-[15px] tracking-[0.08em]">{d.surfaces.lames.aspTitle}</h3>
              <p className="mt-2 text-[12.5px] text-[var(--ss-text-2)]">
        ...
        <Specimen names={["seam"]} note={d.surfaces.seam.note}>
          <Plate elevation="flush">
            {d.surfaces.seam.rows.map((row, i) => (
              <div key={row}>
                {i > 0 ? <Seam /> : null}
                <p className="py-3 text-[13px] text-[var(--ss-text-2)]">{row}</p>
              </div>
            ))}
          </Plate>
        </Specimen>
      ...
      {/* ---------------------------------------------------------------- */}
      <section id="instruments" className="mt-16 scroll-mt-8">
      ...
      {/* ---------------------------------------------------------------- */}
      <section id="controls" className="mt-16 scroll-mt-8">
        ...
        <Specimen names={["plate-checkbox", "plate-switch"]} note={d.controls.toggles.note}>
          <div className="flex flex-wrap items-center gap-[calc(var(--ss-em)*1.5)]">
            ...
            <label className="flex items-center gap-[9px] text-[13px] text-[var(--ss-text-2)] opacity-60">
      ...
      {/* ---------------------------------------------------------------- */}
      <section id="overlays" className="mt-16 scroll-mt-8">
      ...
      {/* ---------------------------------------------------------------- */}
      <section id="structure" className="mt-16 scroll-mt-8">
      ...
      {/* ---------------------------------------------------------------- */}
      <section id="decisions" className="mt-16 scroll-mt-8">
            ...
            <div key={no}>
              ...
              <div className="flex gap-5 py-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-[12.5px] text-[var(--ss-text-2)]">{body}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <section id="install" className="mt-16 scroll-mt-8">
        ...
        <p className="mt-6 max-w-[620px] text-[13.5px] leading-[1.9] text-[var(--ss-text-2)]">
          {d.install.namespaceNote}
        </p>
        <Plate elevation="flush" className="mt-4 overflow-x-auto">
          <code className="block font-[family-name:var(--ss-mono)] text-[12.5px] leading-[1.9] whitespace-pre text-[var(--ss-text-2)]">
            {`"registries": {
...
npx shadcn@latest add @starseam/plate-dialog`}
      ...
      <footer className="mt-16">
        ...
        <p className="mt-2 text-[11.5px] text-[var(--ss-text-3)]">{d.footer.license}</p>
      </footer>
    </main>
  );
}
